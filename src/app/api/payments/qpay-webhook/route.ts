...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!qpayInvoiceId || paymentStatus !== 'PAID') {
      return NextResponse.json({ message: 'Awaiting payment or invalid payload' }, { status: 200 });
    }
    
    // Find the invoice in our database using the QPay invoice ID
    const invoicesRef = collection(db, 'invoices');
    const q = query(invoicesRef, where('qpayInvoiceId', '==', qpayInvoiceId), where('status', '==', 'PENDING'));
