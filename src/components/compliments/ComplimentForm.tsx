...
import { Loader2, ArrowRight, ChevronsUpDown } from 'lucide-react';
...
const FormSchema = z.object({
  text: z.string().min(1, { message: 'Wispr-ээ бичнэ үү.' }).max(500, { message: 'Хамгийн ихдээ 500 тэмдэгт.' }),
...
export function ComplimentForm({ ownerId }: { ownerId: string }) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      // AI filter action
      const result = await submitComplimentAction(data.text);

      if (result.success && result.filteredText && firestore) {
        ...
        addDoc(complimentsRef, complimentData).catch(error => {
           ...
           {userLoading ? (
            <div className="flex justify-center items-center h-28">
             ...
             <Card className="bg-secondary/50 border-dashed">
                <CardHeader>
                  <CardTitle className="text-base">Wispr-ийг илүү утгатай болгохын тулд</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <Button type="button" onClick={handleGoogleSignIn} className="w-full font-bold">
                    <GoogleIcon />
                    Google-ээр үргэлжлүүлэх
                  </Button>
                  <p className="text-xs text-center text-muted-foreground">Таныг илчлэхгүй. Зөвхөн hint-ийг илүү онох болгоход ашиглана.</p>
                </CardContent>
            </Card>
          )}
